...
- El botón "Agregar al carrito" en la página de producto (ticket #18) muestra `AddToCartForm` (selector de talla si `hasVariants`, cantidad) solo si hay sesión de cliente; si no, un link a `/login`. La lista de solo lectura de tallas (`Badge` por talla, "(agotado)") se mantiene aparte porque es lo único que ven los invitados sin sesión.

## Carrito (`src/server/services/cart-service.ts`, ticket #18)

- El carrito requiere cuenta de cliente — **no existe carrito de invitado ni fusión al iniciar sesión**. Decisión consciente, no un olvido: el negocio ya es mayorista/minorista con `Address` ligado a `User`, y checkout va a requerir cuenta de todas formas. `/carrito` está protegido por `src/proxy.ts` (`matcher: ["/admin/:path*", "/carrito/:path*"]`, redirige a `/login` si no hay sesión de `cliente`) y la página además llama a `requireCustomer()` como segunda capa, mismo patrón de defensa en profundidad que usa el panel admin.
...
- `/perfil` existía como destino de redirect desde `/login`/`/registro` desde el ticket de auth, pero no tenía página (404) hasta este ticket. Se aprovechó para construir el CRUD completo de `Address` (modelo ya definido en el schema desde el inicio, sin usar) en vez de solo un placeholder: el checkout va a necesitar direcciones de todas formas. Protegido igual que `/carrito` — `proxy.ts` (`matcher` incluye `/perfil/:path*`) + `requireCustomer()` en cada página/action.
